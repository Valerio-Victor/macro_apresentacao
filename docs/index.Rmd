...
Coordenador Adjunto de Graduação de Curso e Coordenador do **MPA**.
...
- Define-se por **entendimento do funcionamento da economia de um país**:
...
pib_anual <- sidrar::get_sidra(api = '/t/5932/n1/all/v/6561/p/all/c11255/90707/d/v6561%201') %>% 
  janitor::clean_names() %>%  
  dplyr::select(trimestre_codigo, valor) %>% 
  dplyr::mutate(trimestre_codigo = lubridate::yq(trimestre_codigo))

ano_inicial <- lubridate::year(pib_anual$trimestre_codigo[1])
trimestre_inicial <- lubridate::quarter(pib_anual$trimestre_codigo[1])
...
pib_anual_prev <- forecast::forecast(forecast::auto.arima(pib_ts), h = 8)

prev_df <- tibble::tibble(
  tempo = time(pib_anual_prev$mean),
  prev  = as.numeric(pib_anual_prev$mean),
  lower = as.numeric(pib_anual_prev$lower[,2]),
  upper = as.numeric(pib_anual_prev$upper[,2])
)

prev_df <- prev_df %>%
  dplyr::mutate(
    ano = floor(tempo),
    tri = cycle(pib_anual_prev$mean),
    periodo = paste0(ano, " – ", tri, "º trimestre"),
    valor_fmt = scales::percent(prev/100, decimal.mark = ",", accuracy = 0.01),
    lower_fmt = scales::percent(lower/100, decimal.mark = ",", accuracy = 0.01),
    upper_fmt = scales::percent(upper/100, decimal.mark = ",", accuracy = 0.01)
  )
...
hist_df <- hist_df %>%
  dplyr::mutate(
    ...
    tri = cycle(pib_ts),
    periodo = paste0(ano, " – ", tri, "º trimestre"),
    valor_fmt = scales::percent(valor/100, decimal.mark = ",", accuracy = 0.01)
  )
...
graf <- ggplot() +
  ...
  labs(
    x = "Tempo",
    y = "Taxa de Variação do Índice de Volume Trimestral",
    title = "Produto Interno Bruto - PIB"
  ) +
  xaringanthemer::theme_xaringan(text_font_size = 15,
                                 title_font_size = 20) 

graf <- plotly::ggplotly(graf, tooltip = 'text')

widgetframe::frameWidget(graf, 
                         width = '100%', 
                         height = '100%')
...
- A dinâmica ocorrerá da seguinte maneira:
  ...
  **1.** 5 minutos iniciais: Momento para tirar dúvidas e também para aguardar a 
...
- Caso você chegue após a realização da chamada, será registrada falta; no 
entanto, você poderá permanecer e participar normalmente da aula.
...
```{r, echo = F, fig.align = 'center', out.width = '30%'}
knitr::include_graphics(path = 'grupo_wpp.png')
```
